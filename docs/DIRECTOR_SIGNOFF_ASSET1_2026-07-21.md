...
> Filed verbatim by Meridian, 2026-07-21, per the standing rule: Director
> rulings are archived word-for-word, never paraphrased. Received via
> Timothy (PI). Verification anchor: bundle at rhombic commit `638f4a8`,
> archive sha256 `c1891d50…`.
...
* One minor, non-load-bearing note: the D3 distance-only baseline AUC is CV-seed-sensitive (I got 0.686/0.667 vs 0.675/0.713); if that baseline goes in a paper, pin the fold seed and report it, since the margin-over-distance CI is a headline and its lower end depends on the baseline. The full-model result and the margin's existence are not in question.
