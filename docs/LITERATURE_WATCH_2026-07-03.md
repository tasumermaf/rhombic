...
1. **The W2T tension is a feature — but state it precisely** *(corrected 2026-07-03 after full-paper verification)*: on W2T's benchmarks (10,177–20,000 adapters per collection), MLPs over flattened raw factors sit at or near chance (AUROC 47.8–51.6; macro-F1 as low as 2.87% on GoEmotions-LoRA and 7.73% on CelebA-LoRA — note W2T itself scores only 5.67% on GoEmotions, so that collection reflects task hardness, not just representation failure), while symmetry-aware encoders reach ~48–50% macro-F1 on CelebA (Table 1). Caution: **W2T's collections are themselves same-base, same-rank families**, so the regime contrast with our within-family linear separability cannot be attributed to base/rank; the distinguishing variables are label granularity and task structure, and the framing must be re-derived from the D1 bank's own raw-vs-canonicalized comparison before write-up.
