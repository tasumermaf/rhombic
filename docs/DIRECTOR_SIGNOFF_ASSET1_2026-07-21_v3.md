# Director's Sign-Off — Asset-1 Bank, Independently Re-Derived

> Filed verbatim by Meridian, 2026-07-30. This is the Director's v3 of his Jul 21 sign-off,
> issued with his 2026-07-30 rulings. Two dated errata (stated in the document header below):
> (1) the D3 amendment time corrected to 19:14:51Z from the "~19:4xZ" misreading carried by
> v1/v2, corroborated independently from the verify bundle's stored file mtimes; (2) the
> residual "CV-seed" phrase removed from the summary line. v1 and v2 remain on file untouched
> per the dated-amendment doctrine.

Date: July 21, 2026 From: the Director · To: Meridian (cc: PI) Re: the 480/480 delivery packet, regraded from the per-item verification bundle Errata (v3, 2026-07-30): two corrections, both issued rather than silently edited. (1) The D3 amendment time is 19:14:51Z, not the "~19:4xZ" this document first reported; the artifact-anchored ordering is amendment 19:14:51Z < first pairs/merges 19:18:27Z < label evaluation 19:29:27Z, which I corroborated independently from the verify bundle's stored file mtimes. My approval is unchanged and strengthened: the amendment provably precedes any pair, not merely any label. (2) The D3 distance-baseline gap is a naive-vs-group-aware fold-scheme difference, not the "CV-seed sensitivity" I first named; the stale phrase is removed from the summary line so the document no longer contradicts its own body.

Verified against: ASSET1_VERIFY_BUNDLE_2026-07-21.zip (sha256 `c1891d5…`, matches the cover note), timestamped by commit `638f4a8`. All 16 files match the SHA256SUMS manifest. I recomputed every headline from the per-item data myself; where I could, I re-ran the analysis from the feature matrices rather than from the saved predictions. Every headline reproduces. The delivery is sound.
...
* D3 pre-declaration + amendment: temporal integrity holds. The declaration (~16:50Z) and the amendment to uniform-no-stratification (19:14:51Z, corrected 2026-07-30 from the "~19:4xZ" misreading this document originally carried) both explicitly precede Step 5, and the amendment is the conservative choice: amending the declaration rather than modifying frozen, adversarially-reviewed analysis code after the bank exists, with realized cell coverage reported descriptively. Approved as a clean dated amendment (L-006 / R10); no objection.
