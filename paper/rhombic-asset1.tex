...
% TODO(Timothy): confirm author list
\author{%
  TASUMER MAF\\
  Timothy Bielec%
...
choice pinned by an independent Director before the bank completed. (1) Raw
...
raw $\rightarrow$ 0.1521/0.0000 standardized), and shift-controlled transfer
...
tasks costs ${\sim}0.0000$ nats of validation loss; only full-entry
...
independently re-derived from per-item data by the Director. Two of the five
...
locked before the bank completed, every open analytical choice was pinned by
an independent Director in dated rulings, and a completeness interlock
...
covariate-shift artifact; once per-family standardization removes it, task
...
task's bridge costs ${\sim}0.0000$ nats of validation loss, and only
...
fine-grained regimes; midpoint merges only; validation-loss-based labels;
...
\begin{enumerate}
  ...
  family-scale artifact exposed by the family-identity probe (1.0000 raw
  $\rightarrow$ 0.1521/0.0000 standardized), and shift-controlled transfer
  ...
  family, every swap that preserves the identity backbone costs
  ${\sim}0.0000$ nats; only destroying the backbone costs ($+2.8086$ /
  ...
  first result we know of in the weight-only, post-hoc, no-training-access
  regime.
...
W2T (``LoRA Weights Already Know What They Can Do,'' March 2026)
\citep{w2t2026lora} established that LoRA checkpoints are legible objects:
attribute classification and retrieval over collections of 10k+ adapters,
...
fine-grained attribute labels (up to 312 classes) over collections of 10k+
...
vision adapters, a modality difference we note but do not lean on)---so the
...
The $\mathrm{GL}(r)$ gauge is the shared enemy of this line. W2T's
QR$\rightarrow$SVD canonicalization is the lineage our canonical
...
triaged for mergeability with nothing but the files.
...
(\citealp{backdoor2026}, with a 500+-adapter benchmark) and WeightWatcher's
...
overfit-adjacent diagnostics are the neighborhood our D-aux re-verification
...
with a 6-channel identity-initialized bridge (a $6 \times 6$ matrix per
wrapped module---the 36 entry positions that
Section~\ref{sec:d2}'s permutation cells operate on), using plain
...
\paragraph{Director-pinned decisions.} Every analytical choice the card left
open was pinned by an independent Director on 2026-07-06---before the bank
...
The five locked analyses ran against the real bank exactly once, in a fixed
...
Wilson intervals are reported with the standing caveat that LOO folds are
not independent; the permutation $p$ is the calibrated inference. A
...
\texttt{proj\_seed = 0} (88,704 dimensions for Qwen, 50,688 for Llama).
...
between adapters and measuring the val-loss consequence. Stage A (CPU) plans
...
\textbf{either} endpoint by $\ge 5\%$ relative to that endpoint's native
adapter, with the $< 10\%$ degenerate-balance floor triggering a reported
...
group-aware scheme (distance 0.675/0.713). The ${\sim}0.02$--0.05 baseline
...
to within fold-reshuffle noise.

This analysis is scoped precisely against arXiv:2606.19549
\citep{mergeprobe2026}, which owns the training-time form of merge-conflict
prediction (Section~\ref{sec:rw}). D3 is the weight-only, post-hoc
regime---no training access---and claims only that regime.
...
2026-07-20 at ${\sim}$16:50Z, while D2 Stage-B evaluations were in progress
...
At ${\sim}$19:4xZ the same day---still before Step 5 ran, still with zero
pairs, merges, or labels in existence---the declaration was amended.
...
card scoped it ``external''---so \texttt{asset1\_d3\_labels.py} was written
...
The regrade recomputed every headline from per-item rows rather than summary
tables, and for the two highest-risk results went further: the H1 canonical
LOO-SVM was re-run from scratch on the 88,704/50,688-dimension feature
matrices (precomputed-Gram LOO, $C = 1.0$), and the full H2 pipeline was
re-run end-to-end from the spectrum and probe matrices, with the Director
applying the standardization independently and confirming in source that
\texttt{familywise\_standardize} never touches task labels. D2's 14 per-kind
means were reproduced exactly from the 360 per-eval rows per family; the D3
conflict rate and full-model AUC and the D-aux correlations were reproduced
from per-pair and per-run records. Every headline reproduced; the one
note---the D3 distance-baseline fold-scheme sensitivity---is reported in
\S\ref{sec:d3} and wherever the baseline appears. The delivery report itself
...
choice pinned before the bank existed
...
highest-dimensional representation by three orders of magnitude---fail the
...
contradiction: raw weights carry a \emph{weak} recoverable signal (per-class
recall on the code task of 0.40--0.45 drives the null rejection) but come
...
\begin{table}[ht]
\centering
\caption{The triviality control. Raw representations perfectly encode family
identity; per-family standardization removes it.}
\label{tab:trivial}
\begin{tabular}{llcc}
\toprule
...
and 0.0000 (probe). The Director confirmed in source that the
...
binomial $p$ between $7.70\times10^{-98}$ and $1.37\times10^{-96}$.
Condition (i) of the pinned rule fails decisively in both directions, in
both representations; spectrum and probe agree.
...
\begin{table}[ht]
\centering
\caption{D2 penalty matrix (mean val-loss penalty vs native, nats;
360 evals/family).}
\label{tab:d2}
\begin{tabular}{lcc}
\toprule
...
in-distribution loss. This is consistent with the bank's controller-free,
...
realized 14/120 (qwen) and 22/120 (llama) against the ${\sim}16\%$ expected
under uniformity per the pre-declaration.}
\label{fig:f5}
\end{figure}
...
split. The rule held: the conflict rate is 85.8\% (\texttt{frac\_positive}
0.8583, identical at both endpoints), above the floor, so the primary rule
stands and no fallback was used. A conflict rate this high is itself a
finding about midpoint merging in this bank: at $\alpha = 0.5$, most pairs
degrade at least one endpoint by 5\%. Native losses were taken from the
trainer's recorded finals after verifying 0.00000\% divergence against 36
...
\caption{D3 OOF-score separation, full vs distance-only feature sets, per
family.}
\label{fig:f4}
\end{figure}
...
against the group-aware headline's 0.675 / 0.713. The ${\sim}0.02$--0.05
gap is fold-scheme variance on a 2-feature model over 120 points, not seed
...
\paragraph{Interpretive caveat.} Invertible bridges are a gauge on the
...
in-distribution loss structure.
...
The labels runner had no shipped implementation (the card scoped it
...
\paragraph{The gauge, not the information, was the obstacle.} The five
results share one mechanism. In H1, raw adapter weights fail the
...
sub-threshold accuracy shows a weak recoverable trace (code recall
0.40--0.45), but the factorization gauge buries it below the lock. In H2,
...
experiment card scoped it ``external''); the runner written for it was put
...
regimes such as W2T's up-to-312-class attribute label spaces, in either
...
operators, and merges of more than two adapters are outside the measured
claim.

\paragraph{Uniform pair sampling, by dated amendment.} The D3 pair design
was pre-declared (2026-07-20 ${\sim}$16:50Z) with per-cell stratification
over task pairs, then amended (${\sim}$19:4xZ, before Step 5 ran, with zero
pairs, merges, or labels in existence) to the uniform vertex-disjoint
sampler the frozen tool actually implements---the lesser deviation, against
modifying frozen, adversarially-reviewed analysis code after the bank
existed. The Director approved it as a clean dated amendment (L-006 / R10).
...
${\sim}0.02$--0.05 on a 2-feature model over 120 points, not seed noise. The
full-model result and the existence of the margin are not in question, but
...
\texttt{c1891d50\ldots}, all 16 files matching the SHA256SUMS manifest
...
currently local artifacts (the feature matrices are larger than the
repository carries and were transferred out-of-band for the regrade). The
public release surface for the bank, features, and verification bundle is a
pending decision and is not claimed here.

% ===================================================================
% Figures note (from the md's Section 8 placeholder): Tables T2--T7 from
% the outline are realized in-text as Tables 1--6 above; figures F1--F7
% are placed near their first reference in Sections 4--5. T1 (bank design
% table: 2 families x 6 tasks x 40 seeds; steps, val split, geometry;
% generate from the bank_manifest.json campaign block) remains TO
% GENERATE and is not fabricated here.
% TODO(Timothy): decide whether to add T1 from bank_manifest.json.
