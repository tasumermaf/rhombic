...
(1)~le matrici ponte codificano l'identit\`a del task---un SVM lineare
su 336 matrici ponte individuali (112 per task su tre task, ciascuna
una matrice ponte $6 \times 6$ a 36 parametri) classifica il tipo di
task con un'accuratezza leave-one-out del 72,3\% rispetto al 33,3\%
del caso;
...
nell'architettura TeLoRA~\cite{rhombilora_paper1, rhombilora_paper2}),
...
Un SVM lineare addestrato su 336 matrici ponte individuali (112 per
task su tre task, ciascuna una matrice ponte $6 \times 6$ a 36
parametri) classifica il tipo di task con un'accuratezza leave-one-out
del 72,3\%, rispetto al 33,3\% del caso; le distanze inter-task tra
matrici ponte superano quelle intra-task ($p < 10^{-6}$,
Mann-Whitney $U$).
...
riserva.
La matrice ponte fornisce un'impronta compatta del task (36 parametri
per matrice ponte---molto pi\`u piccola di un singolo vettore di stato
nascosto in Qwen2.5-7B; le impronte aggregate di tutti i moduli
classificano meglio, con la proiezione query come modulo singolo pi\`u
discriminativo), uno spazio a bassa
...
TeLoRA addestra congiuntamente tutte e tre le componenti,
...
%
% Didascalia: "Architettura TeLoRA. La matrice ponte M accoppia i
...
Valutiamo le matrici ponte di TeLoRA attraverso sette esperimenti,
...
\texttt{https://github.com/tasumermaf/rhombic}.
...
(1)~LoRA standard $r\!=\!24$, (2)~TeLoRA $r\!=\!24$ con matrice
ponte identit\`a congelata, (3)~TeLoRA $r\!=\!24$ con matrice ponte
addestrabile (inizializzazione identit\`a), (4)~LoRA standard
$r\!=\!48$ (budget di parametri $2\times$), e (5)~TeLoRA
...
2 & TeLoRA (congelata)    & 24 & 6 & $I$ congelata & 3.268.608 \\
3 & TeLoRA (addestrabile) & 24 & 6 & Addestrabile  & 3.270.624 \\
4 & LoRA Standard          & 48 & 6 & $I$ congelata & 6.537.216 \\
5 & TeLoRA (cubica)     & 24 & 3 & Addestrabile  & 3.269.112 \\
\bottomrule
\end{tabular}
\end{table}
...
confermando che TeLoRA con matrice ponte identit\`a congelata
...
\paragraph{Configurazione.} Addestriamo TeLoRA ($r\!=\!24$, FCC,
...
Addestriamo TeLoRA ($r\!=\!24$, FCC) su Qwen2.5-7B-Instruct
...
\paragraph{Configurazione.}
Addestriamo TeLoRA ($r\!=\!24$, FCC, matrice ponte addestrabile)
...
(\S\ref{sec:exp-direction}), addestriamo TeLoRA ($r\!=\!24$, FCC,
...
una configurazione TeLoRA identica ($r\!=\!24$, FCC, matrice ponte
...
Un SVM lineare sulle 336 matrici ponte individuali (112 per task su
tre task, ciascuna una matrice ponte $6 \times 6$ a 36 parametri)
raggiunge il 72,3\% di accuratezza leave-one-out a tre vie (caso:
33,3\%). La coppia codice--matematica \`e la pi\`u vicina in distanza
tra matrici ponte (Tabella~\ref{tab:fingerprint-distances}) e di
conseguenza la pi\`u difficile da separare.

\paragraph{Discriminativit\`a per modulo.}
Addestrando classificatori separati sulle matrici ponte di ciascun
modulo (84 campioni per modulo, 36 parametri ciascuno) si ottiene la
classifica dei moduli: q\_proj 69,0\%, o\_proj 60,7\%, k\_proj 58,3\%,
v\_proj 51,2\% (Tabella~\ref{tab:subset-class}). La proiezione query
\`e il modulo singolo pi\`u discriminativo per il task, coerentemente
con la sua distanza media inter-task pi\`u elevata (0,16076) e la sua
forza di accoppiamento pi\`u elevata (\S\ref{sec:exp-anatomy-7b}).
Nessun modulo singolo, tuttavia, supera il classificatore aggregato:
le impronte di tutti i moduli raggiungono il 72,3\%, superando il
miglior modulo singolo di 3,3 punti percentuali. L'identit\`a del task
\`e distribuita tra le matrici ponte della testa di attenzione anzich\'e
concentrata in un sottoinsieme sparso.

\begin{table}[t]
\centering
\caption{Accuratezza della classificazione per modulo (LOO SVM, 3 vie;
ogni campione \`e una singola matrice ponte $6 \times 6$ a 36
parametri). Nessun modulo singolo supera il classificatore aggregato
di tutti i moduli; la proiezione query \`e il modulo singolo pi\`u
discriminativo.}
\label{tab:subset-class}
\small
\begin{tabular}{@{}lrrr@{}}
\toprule
Insieme di impronte & Campioni & Param./ponte & Accuratezza LOO \\
\midrule
Tutti i moduli (Q+K+V+O) & 336 & 36 & \textbf{72,3\%} \\
Solo q\_proj & 84 & 36 & 69,0\% \\
Solo o\_proj & 84 & 36 & 60,7\% \\
Solo k\_proj & 84 & 36 & 58,3\% \\
Solo v\_proj & 84 & 36 & 51,2\% \\
\bottomrule
\end{tabular}
\end{table}
...
e ancora in crescita. All'ultimo checkpoint, le impronte aggregate di
tutti i moduli classificano il tipo di task a tre vie al 72,3\%
(q\_proj da solo: 69,0\%). Questa traiettoria esclude
...
al 72,3\% di accuratezza leave-one-out (caso: 33,3\%).
...
una configurazione TeLoRA a rango 24 applicata a quattro moduli
...
Esperimento~1: TeLoRA con matrice ponte congelata corrisponde alla
...
Un SVM lineare su 336 matrici ponte individuali (112 per task su tre
task, ciascuna una matrice ponte $6 \times 6$ a 36 parametri)
classifica il tipo di task con un'accuratezza leave-one-out del
72,3\% (caso: 33,3\%). Ogni campione d'impronta costa 36 parametri
($6 \times 6$)---molto pi\`u piccolo di un singolo vettore di stato
nascosto in Qwen2.5-7B (4.096 dimensioni). I classificatori per
modulo ordinano q\_proj (69,0\%) sopra o\_proj (60,7\%), k\_proj
(58,3\%) e v\_proj (51,2\%): la proiezione query \`e il modulo singolo
pi\`u discriminativo, ma nessun modulo singolo supera il
classificatore aggregato di tutti i moduli.
...
degli adattatori: memorizzare le matrici ponte di un adattatore come
sua impronta del task e utilizzarle per indirizzare le richieste in
arrivo verso l'adattatore corretto senza inferenza. L'aggregazione di
tutti i moduli fornisce la migliore classificazione (72,3\%); se lo
spazio di memorizzazione \`e vincolato, le matrici ponte q\_proj sono
il sottoinsieme a modulo singolo pi\`u informativo (69,0\% da sole),
sebbene ogni modulo porti segnale del task (tutti i classificatori per
modulo superano il caso). Le matrici ponte
...
\textbf{Tre task.} La classificazione a tre vie al 72,3\% \`e
...
vincolata (generazione di codice). Un SVM lineare su 336 matrici
ponte individuali---112 per task su tre task, ciascuna una matrice
ponte $6 \times 6$ a 36 parametri---classifica il tipo di task con
un'accuratezza leave-one-out del 72,3\% (caso 33,3\%) senza
...
La proiezione query \`e il modulo singolo pi\`u discriminativo
(69,0\% da sola)---codifica l'aspetto pi\`u specifico per il task
dell'attenzione: cosa cercare---sebbene nessun modulo singolo superi
il classificatore aggregato.
...
confrontando le impronte delle matrici ponte (36 parametri per matrice
ponte) di un
...
disponibili presso \url{https://github.com/tasumermaf/rhombic} sotto
