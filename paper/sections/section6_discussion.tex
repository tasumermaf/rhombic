...
TeLoRA configuration applied to four target modules across 28
...
Experiment~1 result: frozen-bridge TeLoRA matches standard LoRA
...
A linear SVM over 336 individual bridge matrices (112 per task
across three tasks, each a 36-parameter $6 \times 6$ bridge)
classifies task type at 72.3\% leave-one-out accuracy (chance:
33.3\%). Each fingerprint sample costs 36 parameters ($6 \times
6$)---far smaller than a single hidden state vector in Qwen2.5-7B
(4{,}096 dimensions). Per-module classifiers rank q\_proj (69.0\%)
above o\_proj (60.7\%), k\_proj (58.3\%), and v\_proj (51.2\%): the
query projection is the most discriminative single module, but no
single module outperforms the pooled all-module classifier.
...
vs.\ $R^2 = 0.948$ for alpaca$\leftrightarrow$code). The
...
store an adapter's bridge matrices as its task fingerprint and use
them to route incoming requests to the correct adapter without
inference. Pooling all modules gives the best classification
(72.3\%); if storage is constrained, q\_proj bridges are the most
informative single-module subset (69.0\% alone), though every module
carries task signal (all per-module classifiers exceed chance).
Q-proj bridges require task-specific handling during composition.
...
\textbf{Three tasks.} Three-way classification at 72.3\% is
...
code and math for 2K each. The monotonic relationship
...
Papers~1--2~\cite{rhombilora_paper1, rhombilora_paper2} meets
...
The lattice geometry provides two distinct resources.
\textbf{Connectivity}---the algebraic advantage of more coupling
pathways---emerges naturally from unsupervised training. Six
channels with FCC-topology initialization develop $4.6\times$
more algebraic connectivity than three channels with cubic
initialization at matched parameter budgets (\S\ref{sec:exp-anatomy-1b}).
The optimizer independently discovers that the FCC arrangement of
coupling pathways is useful, without any supervision about channel
semantics. This is the capacity dimension: the topology determines
\emph{how much} coupling the bridge develops.

\textbf{Direction}---the association of specific channels with
specific lattice directions---cannot emerge from data alone. Two
independent null results confirm this: structured geometric data
produces Co/Cross $= 1.002$ (\S\ref{sec:exp-direction}); a
$10\times$ higher bridge learning rate produces Co/Cross
$\approx 1.0$ (\S\ref{sec:exp-bridge-lr}). However, 500 steps
of contrastive supervision installs directional preference
(Co/Cross $= 2{,}660\times$) that persists as a stable geometric
memory: after 6{,}000 unsupervised training steps, the ratio
remains at $16\times$---never approaching the null baseline
(\S\ref{sec:exp-contrastive}). During the same unsupervised
period, Fiedler connectivity rises $3.7\times$ independently.
The bridge simultaneously develops more overall coupling
\emph{and} retains the directional structure it was taught.

This two-resource separation clarifies the relationship between
lattice topology and neural adaptation. The FCC lattice provides
an \emph{addressable geometric coordinate system}: its six
direction pairs can be given semantic meaning through contrastive
supervision, and that meaning persists through subsequent training.
Connectivity is the free resource---any topology provides it, but
FCC provides $4.6\times$ more. Direction is the expensive
resource---it must be explicitly taught. Once taught, the lattice
geometry ensures it has somewhere stable to live.
...
A linear SVM over 336 individual bridge matrices---112 per task
across three tasks, each a 36-parameter $6 \times 6$
bridge---classifies task type at 72.3\% leave-one-out accuracy
(chance 33.3\%) without
...
The query projection is the most discriminative single module
(69.0\% alone)---it encodes the most task-specific aspect of
attention: what to look for---though no single module outperforms
...
The bridge also exhibits \emph{geometric memory}. Contrastive
pre-training installs directional preference into the bridge's
coupling structure (co-planar/cross-planar ratio $> 2{,}600\times$
after 500 supervised steps). When supervision is removed, the
directional signal decays but never reaches the null baseline:
after 6{,}000 unsupervised steps, the ratio stabilizes near
$16\times$---while two independent control experiments confirm
that unsupervised training alone produces a ratio of $\approx 1.0$.
During the same unsupervised period, overall bridge connectivity
rises $3.7\times$ independently: the optimizer discovers the utility
of cross-channel coupling on its own, but cannot discover
\emph{which} channels to couple preferentially. The bridge
separates two resources that the lattice topology provides:
connectivity (free, emerges from training) and direction (expensive,
requires teaching, persists once taught).
...
capture finer coupling structure. The geometric memory finding opens
a new direction: if brief contrastive warmup can install persistent
geometric structure, the bridge may serve not only as a passive
diagnostic but as a site for injecting inductive biases that shape
subsequent unsupervised training. The most immediate practical
...
adapter's bridge fingerprints (36 parameters per bridge) against
stored fingerprints, without running any of the adapters at
inference time.

\paragraph{Availability.}
Source code: \url{https://github.com/tasumermaf/rhombic}.\\
