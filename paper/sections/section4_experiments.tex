...
We evaluate TeLoRA's bridge matrices across seven experiments,
progressing from single-adapter structural analysis through directional
supervision and geometric memory to multi-task fingerprinting and
bridge-level adapter composition. All experiments use the
...
scripts are available at \texttt{https://github.com/tasumermaf/rhombic}.
...
2 & TeLoRA (frozen)    & 24 & 6 & Frozen $I$ & 3,268,608 \\
3 & TeLoRA (learnable) & 24 & 6 & Learnable  & 3,270,624 \\
4 & Standard LoRA          & 48 & 6 & Frozen $I$ & 6,537,216 \\
5 & TeLoRA (cubic)     & 24 & 3 & Learnable  & 3,269,112 \\
\bottomrule
\end{tabular}
\end{table}
...
TeLoRA with a frozen identity bridge recovers standard LoRA exactly.
...
established by graph-theoretic benchmarks~\cite{rhombilora_paper1}.
...
\paragraph{Setup.} We train TeLoRA ($r\!=\!24$, FCC, learnable bridge)
...
\paragraph{Setup.}
We train TeLoRA ($r\!=\!24$, FCC) on Qwen2.5-7B-Instruct using a
...
directional preference requires explicit supervision of the bridge's
coupling structure---we test this directly in
\S\ref{sec:exp-contrastive}.

% ---------------------------------------------------------------------------
\subsection{Contrastive Bridge Pre-Training}
\label{sec:exp-contrastive}

\paragraph{Motivation.}
The null result in \S\ref{sec:exp-direction} established that channel
assignment is arbitrary: the bridge has no mechanism to associate specific
channels with specific lattice directions from data alone. We test whether
\emph{explicit supervision}---a contrastive loss that rewards co-planar
coupling and penalizes cross-planar coupling---can install directional
preference, and whether such preference persists after supervision is
removed.

\paragraph{Setup.}
We train TeLoRA ($r\!=\!24$, FCC, learnable bridge) on
Qwen2.5-7B-Instruct using Alpaca-cleaned in two phases on a RunPod
RTX 3090:

\begin{enumerate}
\item \textbf{Contrastive warmup} (500 steps): The training loss
$\mathcal{L} = \mathcal{L}_\text{LM} + w_c \mathcal{L}_\text{contrastive}$
combines the standard language modeling objective with a contrastive term
that maximizes coupling between co-planar channel pairs (those sharing
4 octahedral vertices) relative to cross-planar pairs (sharing 2). The
contrastive weight ramps linearly from 0 to $w_c = 0.01$ over 500 steps.

\item \textbf{Unsupervised continuation} (steps 501--10,000+): The
contrastive loss is removed entirely ($w_c = 0$). Training continues
with the language modeling objective alone. The bridge receives gradients
only through the standard
$\partial\mathcal{L}_\text{LM}/\partial M_{ij}$ pathway.
\end{enumerate}

\noindent
Bridge matrices are checkpointed every 100 steps. The
co-planar/cross-planar coupling ratio (Co/Cross) provides the primary
diagnostic: at what rate does the supervised directional signal decay
under unsupervised training, and does it reach the null baseline of 1.0
established by Experiments~2.5 and~2.7?

\paragraph{Contrastive warmup.}
At step 500, the bridge exhibits massive directional preference:
Co/Cross $= 2{,}660\times$ (Table~\ref{tab:geometric-memory}). The
contrastive loss successfully installs the geometric semantics that
data-driven training (\S\ref{sec:exp-direction}) could not: co-planar
pairs carry orders of magnitude more coupling weight than cross-planar
pairs.

\paragraph{Geometric memory.}
After supervision is removed at step 500, the directional signal decays
rapidly but \emph{never reaches the null baseline}
(Table~\ref{tab:geometric-memory}). By step 5{,}000 (4{,}500 unsupervised
steps), Co/Cross has decayed from $2{,}660\times$ to $18.6\times$---a
99.3\% reduction in magnitude, but still $18.6\times$ above the null
baseline of $1.0$ established by two independent experiments
(\S\ref{sec:exp-direction}, \S\ref{sec:exp-bridge-lr}). By step 6{,}500,
the ratio stabilizes near $16\times$, with successive 500-step decrements
shrinking below 0.5.

\begin{table}[t]
\centering
\caption{Geometric memory: co-planar/cross-planar coupling ratio (Co/Cross),
Fiedler value, and validation loss after 500 steps of contrastive warmup
followed by unsupervised training. The directional signal decays toward but
never reaches the null baseline ($\approx$1.0). Training was ongoing at
time of writing; the asymptotic trend is already clear.}
\label{tab:geometric-memory}
\small
\begin{tabular}{@{}rlrrr@{}}
\toprule
Step & Phase & Co/Cross & Fiedler $\lambda_2$ & Val Loss \\
\midrule
0     & Init            & 1.000    & 0.000 & --- \\
500   & Warmup end      & 2{,}660  & ---   & --- \\
3{,}500 & Unsupervised  & 56.0   & 0.007 & --- \\
5{,}000 & Unsupervised  & 18.6   & 0.022 & 0.301 \\
5{,}500 & Unsupervised  & 17.3   & 0.024 & 0.297 \\
6{,}500 & Unsupervised  & 16.1   & 0.026 & 0.295 \\
\midrule
\multicolumn{2}{@{}l}{\textit{Null baselines}} \\
\quad Exp 2.5 & Geometric data    & 1.002  & 0.030 & --- \\
\quad Exp 2.7 & High bridge LR    & $\approx$1.0 & --- & --- \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Connectivity emerges independently.}
While the directional signal decays, the bridge's \emph{overall}
connectivity rises monotonically. Fiedler value increases from $0.007$ at
step 3{,}500 to $0.026$ at step 6{,}500---a $3.7\times$ increase driven
entirely by unsupervised gradients. Deviation from identity likewise
increases ($0.151 \to 0.193$), indicating that the bridge becomes
\emph{more} expressive, not less, during the unsupervised phase. Validation
loss improves steadily ($0.307 \to 0.295$). The bridge is
reorganizing---trading excess directional specificity for task-useful
coupling---while retaining a persistent geometric memory of the directional
structure it was shown during warmup.

\paragraph{Revised finding.}
The original finding L-001 (``the bridge cannot learn directional preference
from data alone,'' \S\ref{sec:exp-direction}) must be revised: \emph{the
bridge cannot discover direction from data alone, but once shown direction
through explicit supervision, it retains a stable geometric memory that
thousands of unsupervised training steps cannot erase.} Direction requires
teaching; once taught, it persists.

% ---------------------------------------------------------------------------
\subsection{Bridge Learning Rate Control}
\label{sec:exp-bridge-lr}

\paragraph{Setup.}
To rule out optimization dynamics as an alternative explanation for the
directional null (\S\ref{sec:exp-direction}), we train TeLoRA
($r\!=\!24$, FCC, learnable bridge) on Qwen2.5-7B-Instruct using
Alpaca-cleaned for 3{,}000 steps on a RunPod RTX 3090, with a separate
learning rate for bridge parameters set to $10\times$ the base adapter
rate ($2 \times 10^{-3}$ vs.\ $2 \times 10^{-4}$). No contrastive loss
is applied. If the directional null in Experiment~2.5 resulted from
insufficient bridge learning rate---the bridge simply not updating fast
enough relative to the projection matrices---then amplifying the bridge
gradient should produce directional signal.

\paragraph{Null result.}
The co-planar/cross-planar ratio remains at $\approx$1.0 throughout
training---indistinguishable from the identity initialization and from
the Experiment~2.5 null (Table~\ref{tab:geometric-memory}, bottom rows).
Increasing the bridge learning rate by $10\times$ does not induce
directional preference. Combined with the Experiment~2.5 result (standard
LR, geometric data) and the Experiment~2.6 result (standard LR,
contrastive supervision), this establishes that directional preference
requires \emph{explicit contrastive supervision}, not faster optimization
or structured data.
...
examples, 2K steps, RunPod RTX 4090). All use identical TeLoRA
...
A linear SVM over the 336 individual bridge matrices (112 per task
across three tasks, each a 36-parameter $6 \times 6$ bridge) achieves
72.3\% three-way leave-one-out accuracy (chance: 33.3\%). The
code--math pair is the closest in bridge distance
(Table~\ref{tab:fingerprint-distances}) and correspondingly the
hardest to separate.

\paragraph{Per-module discriminability.}
Training separate classifiers on each module's bridges (84 samples per
module, 36 parameters each) ranks the modules: q\_proj 69.0\%, o\_proj
60.7\%, k\_proj 58.3\%, v\_proj 51.2\%
(Table~\ref{tab:subset-class}). The query projection is the most
task-discriminative single module, consistent with its highest mean
between-task distance (0.16076) and highest coupling strength
(\S\ref{sec:exp-anatomy-7b}). No single module, however, outperforms
the pooled classifier: all-module fingerprints achieve 72.3\%,
exceeding the best single module by 3.3 percentage points. Task
identity is distributed across the attention head's bridges rather
than concentrated in any sparse subset.

\begin{table}[t]
\centering
\caption{Per-module classification accuracy (LOO SVM, 3-way; each
sample is a single 36-parameter $6 \times 6$ bridge). No single module
outperforms the pooled all-module classifier; the query projection is
the most discriminative single module.}
\label{tab:subset-class}
\small
\begin{tabular}{@{}lrrr@{}}
\toprule
Fingerprint set & Samples & Params/bridge & LOO Accuracy \\
\midrule
All modules (Q+K+V+O) & 336 & 36 & \textbf{72.3\%} \\
q\_proj only & 84 & 36 & 69.0\% \\
o\_proj only & 84 & 36 & 60.7\% \\
k\_proj only & 84 & 36 & 58.3\% \\
v\_proj only & 84 & 36 & 51.2\% \\
\bottomrule
\end{tabular}
\end{table}
...
and still climbing. At the final checkpoint, pooled all-module
fingerprints classify three-way task type at 72.3\% (q\_proj alone:
69.0\%). This trajectory rules out any initialization
...
and deviation alongside the train--validation loss gap on a RunPod RTX 4090.

Training loss drops to 0.012 while validation loss plateaus at 1.51 by
step 10K (gap $= +1.50$), confirming severe overfitting. Both bridge
spectral properties correlate strongly with the train--validation gap:
deviation ($r = 0.888$, $p = 7.3 \times 10^{-35}$) and Fiedler value
($r = 0.825$, $p = 5.6 \times 10^{-26}$). Both exceed the $r > 0.7$
threshold.

A phase transition in bridge connectivity occurs at step 400: the
maximum step-to-step Fiedler change ($0.019$) exceeds the median
change ($2.3 \times 10^{-5}$) by $807\times$, coinciding with the
early rapid descent in training loss. After this initial reconfiguration,
bridge spectral properties evolve monotonically alongside the growing
generalization gap. The bridge does not predict overfitting onset
\emph{in advance}---it tracks the same gradient signal that drives the
loss gap---but it provides a compact, inference-free summary of adapter
state that could complement loss-based monitoring in practice.
