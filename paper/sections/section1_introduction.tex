...
in the TeLoRA architecture~\cite{rhombilora_paper1, rhombilora_paper2}),
the bridge adds 36 parameters per adapter layer---a $0.02\%$ overhead
relative to total adapter parameters.
When the bridge is the identity matrix $I_C$, the architecture
reduces exactly to standard LoRA.

Through seven experiments spanning bridge anatomy, task fingerprinting,
adapter merging, and contrastive pre-training on Qwen2.5 (1.5B and 7B),
we establish four findings:

\paragraph{(a) Task fingerprinting.}
Bridge matrices encode task identity.
A linear SVM trained on 336 individual bridge matrices (112 per task
across three tasks, each a 36-parameter $6 \times 6$ bridge)
classifies task type at 72.3\% leave-one-out accuracy, compared to
33.3\% chance; between-task bridge distances exceed within-task
distances ($p < 10^{-6}$, Mann-Whitney $U$).
...
(alpaca$\leftrightarrow$code, $R^2 = 0.948$) but non-linear for
dissimilar pairs (code$\leftrightarrow$math, $R^2 = 0.782$), with
...
\paragraph{(d) Geometric memory.}
Contrastive bridge pre-training installs directional preference
(co-planar/cross-planar ratio $> 2{,}600\times$ after 500 supervised
steps) that persists as a stable geometric memory after supervision is
removed: the ratio decays to $16\times$ over 6{,}000 unsupervised steps
but never reaches the null baseline of $1.0$ established by two
independent control experiments. During the same unsupervised period,
overall bridge connectivity rises $3.7\times$ independently---the
optimizer discovers the utility of cross-channel coupling on its own,
while retaining the directional structure it was taught.
...
(co-planar/cross-planar ratio $= 1.002$, $p = 0.474$), confirmed by
a separate bridge learning rate control ($10\times$ LR, Co/Cross
$\approx 1.0$).
...
\emph{which} channels to prefer---unless explicitly taught.
...
The bridge provides a compact task fingerprint (36 parameters per
bridge---far smaller than a single hidden state vector in
Qwen2.5-7B; pooled all-module fingerprints classify best, with the
query projection the most discriminative single module), a
low-dimensional space for adapter composition
...
adaptation.
TeLoRA trains all three components jointly, enabling the bridge
