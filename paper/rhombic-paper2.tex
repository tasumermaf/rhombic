...
integers (range, mean and standard deviation withheld at the corpus boundary) derived from a fixed transliteration protocol applied to a 24-element
...
high variance and a heavy right tail (the coefficient of variation
and skewness are withheld at the corpus boundary), properties shared
with many real-world weight distributions. All lattice-scale findings
...
CPU-bound). 334 tests verify library correctness. Source code and
...
vertex star:
\begin{itemize}
  \item $67 \to$ vertex 7: one edge is a multiple of $67$
  \item $29 \to$ vertex 0: one edge is a multiple of $29$
  \item $19 \to$ vertex 4: one edge is a multiple of $19$
  \item $17 \to$ vertex 3: one edge is a multiple of $17$
  \item $11 \to$ vertex 5: one edge is a multiple of $11$
\end{itemize}
...
bridge matrix whose channels follow polytope face-pair topology
adds cross-rank mixing at negligible parameter cost.
This ``TeLoRA'' (Topology-Enhanced LoRA) architecture is validated in the
companion paper~\cite{bielec2026bridge}, where polytope-derived bridges
produce coherence-to-cross ratios exceeding $10^4$:1.
...
Package: \url{https://pypi.org/project/rhombic/} (v0.3.0, 334 tests,
