...
%      approved by PI — queued on Hermes. Original finding below for the record:
...
(three same-seed runs; pairwise trajectory correlation
$r \geq 0.995$). The bridge is a programmable
...
\begin{enumerate}
  ...
  \item \textbf{Reproducibility.} $n = 8$ tesseract contrastive
    replicates across three same-seed runs: pairwise Fiedler
    trajectory correlation $r \geq 0.9994$ (coupling-ratio
    $r \geq 0.995$) at all matched steps, one run recovered from its
    training log after a crash-era overwrite (disclosed in
...
\begin{itemize}
  ...
  \item \textbf{Tesseract} ($n = 8$, 4D): four $2 \times 2$ blocks,
    ...
    split. Reproducible across three same-seed runs (Fiedler
    $r \geq 0.9994$).
\end{itemize}
