...
%   1. [RESOLVED 2026-07-07 — scope-corrected after Director residual pass]
%      RETRACTED HOLLY DATA. Jul-6 fix stripped the Wan-14B scale + unanchored
%      82,154:1 aggregate from THIS file only; the Jul-6 "RESOLVED" mark was
%      premature — the Director's residual pass found the retracted Holly
%      PERFORMANCE numbers (3.8% loss / 9.15 GB / 6% faster) still live in the
%      PAPER 3 body at 4 sites (+2 more found on sweep: PAPER3_DRAFT_COMBINED,
%      and 1.552/1.493 val-loss cells in three Holly TABLE rows). ALL purged
%      2026-07-07 per his surgical fix: performance clauses struck, null-topology
%      control (0% BD, rho=1.07:1) KEPT (the tracker's do-not-cite is scoped to
%      the three performance numbers). Working docs (5) + VIDEO_CREATIVE_BRIEF
%      carry RETRACTED markers; LITERATURE_WATCH already self-annotated; stale
%      compiled paper4-main.pdf UNTRACKED from git (pre-purge text baked in);
%      body grep for 3.8/9.15/6%-faster now returns ZERO across paper3+paper4.
%      84.5% adjacent debt checked: already absent from paper3 body.
...
%   5. [RESOLVED 2026-07-07 — rewrite APPLIED with PI + Director sign-off]
%      T-001 reproducibility rewritten around the surviving evidence: three
%      same-seed runs (T-001r1 log-recovered partial / T-001r1b restart 7,100 /
%      T-001r2 10,000), pairwise Fiedler r >= 0.9994, co/cross r >= 0.995, max dev
%      5-10%; overwrite disclosed in sec:reproducibility + tab footnote; destroyed
%      endpoint (5,395:1 @2,700) mentioned only as unverifiable contemporaneous
%      record; lm-eval merge claim relabeled to the original adapter with its
%      pre-overwrite eval timestamp. All numbers trace to results/T-001-full*/
%      + results/t001-provenance/recovery.json. Optional 10K re-run (T-001r3)
%      approved by PI — queued on Hermes.
%      [2026-07-10: T-001r3 COMPLETE (Hermes RTX 4090, 10,000 steps, seed 42).
%       Endpoint (results/T-001-full-r3/results.json checkpoints[-1], step 10000):
%       co/cross 41,812:1, Fiedler 1.93e-4, val 0.4017. 0.596% rel dev vs r2's
%       41,564 at step 10K — CROSS-HARDWARE (r2 ran local RTX 6000 Ada). Added as
%       the fourth same-seed run: tab:tesseract row + sec:reproducibility endpoint
%       paragraph; counts three->four in abstract/intro/conclusion + Hardware
%       appendix. Endpoint-only confirmation — no new pairwise trajectory r
%       computed for r3, so tab:reproducibility (3-run pairwise) is unchanged.
%       Dated addition per L-006.]
%      Original finding below for the record:
%      [was 2026-07-07 — ROOT CAUSE FOUND, RESOLUTION PROPOSED:]
%      T-001 REPRODUCIBILITY: the original T-001r1 (2,700 steps; 5,395:1 @2700) was
%      DESTROYED on 2026-03-16 21:54 when T-001-full was relaunched fresh (same seed
%      42) after the Mar-15 machine hang, overwriting its results.json (proof: the
%      relaunch's values at steps <=1600 do not match the frozen log's originals).
%      Its stdout log froze at step 1700; Hermes carries no tesseract artifacts.
%      RECOVERED: 16 original trajectory points (steps 0-1600, Fiedler + co/cross)
%      extracted from T-001-full.log — scripts/t001_log_recovery.py +
%      results/t001-provenance/recovery.json. Three-way same-seed agreement
%      (original-from-log vs Mar-16 restart vs r2): co/cross r 0.995-0.997 (max dev
%      5-10%), Fiedler r >= 0.9994 for ALL pairs — the destroyed run is mutually
%      consistent with both survivors. PROPOSED FIX (pending PI + Director sign-off):
%      rewrite the reproducibility passage around the surviving evidence — three
%      same-seed runs, pairwise Fiedler r >= 0.9994, co/cross r 0.995-0.997 — and
%      replace tab:tesseract's r1 row (2,700 / 5,395:1 / .00070, destroyed artifact)
%      with the on-disk restart (7,100 steps) plus the log-recovered partial,
%      disclosing the overwrite. The old r=1.0000 / 3.5% / "34 checkpoints" specifics
%      reference the destroyed artifact and cannot ship. DO NOT edit until agreed.
...
(four same-seed runs, including a cross-hardware endpoint replication;
pairwise trajectory correlation $r \geq 0.995$). The bridge is a programmable
...
\begin{enumerate}
  ...
  \item \textbf{Reproducibility.} $n = 8$ tesseract contrastive
    replicates across four same-seed runs: three with overlapping
    feedback logs track at pairwise Fiedler trajectory correlation
    $r \geq 0.9994$ (coupling-ratio $r \geq 0.995$) at all matched
    steps---one recovered from its training log after a crash-era
    overwrite---and a fourth, on different hardware, reproduces the
    step-10{,}000 endpoint to $0.6\%$ (disclosed in
...
\begin{itemize}
  ...
  \item \textbf{Tesseract} ($n = 8$, 4D): four $2 \times 2$ blocks,
    ...
    split. Reproducible across four same-seed runs (Fiedler
    $r \geq 0.9994$ across the three with overlapping logs; a fourth
    reproduces the endpoint to $0.6\%$ on different hardware).
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Hermes:} NVIDIA RTX 4090 (16\,GB VRAM). Used for
    ...
    octahedral (O-001), seed replication (Seed-43, Seed-44), and the
    % Dated edit 2026-07-10: T-001r3 added (cross-hardware fourth run).
    fourth tesseract replicate (T-001r3), whose cross-hardware
    endpoint agreement with the local T-001r2 is reported in
    Section~\ref{sec:reproducibility}.
\end{itemize}
