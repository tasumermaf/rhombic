...
%   4. [PARTIAL 2026-07-06] OCTAHEDRAL HOLE: FO-001 262,920:1 is now anchored
%      locally (results/octahedral-hermes-anchor/FO-001/, ratio confirmed in the
%      training log at step 10000). O-001 473,622:1 STILL unanchored — its Hermes
%      results.json carries a NULL co_cross_ratio throughout (the 473,622 came from
%      a post-hoc block analysis, not the training log); must be recomputed from the
%      O-001 adapter before the n=4 endpoint of the inverse-n ordering (474K>70K>42K)
%      can be trusted.
%   5. TESSERACT SPECIFICS wrong vs disk: r=1.0000 holds (recomputed 0.99995),
%      but max-deviation is 5.69% NOT 3.5%, and the overlap is 72 steps (neither
%      34 nor 6). Correct the specifics; the reproducibility claim itself stands.
%   6. BM-001 aggregate "Mean" cells (Std 0.6970 / TeLoRA 0.6982) are
%      NON-REPRODUCIBLE — true 4-metric means are 0.7221 / 0.7232. The headline
%      Delta +0.0012 survives ONLY because the ~0.025 error cancels. Fix the
%      Mean cells (per-benchmark table is exact; independent of the C6b freeze).
