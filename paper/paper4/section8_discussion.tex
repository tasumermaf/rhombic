...
Loss is identical to three significant figures ($\sim$0.06\% difference,
within noise). The bridge adds 2{,}016~parameters---36~per bridge
matrix across 56~target projections---representing 0.06\% of the
trainable parameter budget. The 36.2\% training time overhead is the
cost of training the learnable multi-channel bridge---the only
configuration difference from the standard-LoRA baseline, which carries
the same bridge frozen at identity. This P0 comparison runs no
Steersman, so none of the overhead is control-law evaluation; the
Fiedler eigenvalue is logged only at checkpoint intervals and
contributes negligibly.

Trained under language-model loss alone---no Steersman, and neither
spectral nor contrastive pressure---the bridge stays close to its
identity initialization: mean Fiedler~$= 0.038$, co-axial/cross-axial
ratio~$\approx 1$:1 (isotropic, no directional preference), and mean
deviation from identity~$= 0.07$ (Figure~\ref{fig:p0_proof}). Without
contrastive specification the bridge acquires no block structure---the
unprogrammed baseline. At Fiedler 0.038 it stays even nearer identity
than the spectral-only runs of Section~\ref{sec:attractor} (which apply
a spectral pressure this pure-LM run does not), both within the
...
\begin{figure}[t]
  ...
  \caption{P0 head-to-head comparison on Qwen~2.5-1.5B-Instruct.
    ...
    Top right: training time comparison (36.2\% overhead from training
    the learnable bridge---the only difference from the frozen-bridge
    baseline; this comparison runs no Steersman). Middle: bridge matrix
    evolution at
...
and defaults to a near-initialization resting state when no topology is specified.
